...
/* ===== LOGIN GUARD (ADD THIS HERE) ===== */
function requireLoginOrRedirect() {
    if (window.BLINKHUB_IS_LOGGED_IN === false) {
    ...
    cart.forEach((item, idx) => {
        ...
        tr.innerHTML = `
            ...
            <td class="cart-qty">
                ...
                <button class="qty-btn plus" data-id="${item.id}" data-stock="${item.stock}">+</button>
            </td>
...
document.addEventListener("click", (e) => {
    ...
    const stock = cart[itemIndex].stock;

    if (cart[itemIndex].qty >= stock) {
        showToast(`Only ${stock} quantity remaining`);
        return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let stock = parseInt(ctrl.dataset.stock);

    let existing = cart.find(item => item.id === id);

    if (existing && existing.qty >= stock) {
        showToast(`Only ${stock} quantity available`);
        return;
    }
    ...
    if (existing) {
        ...
        existing = { id, name, price, qty: 1, stock: stock };
        cart.push(existing);
    }
...
ctrl.querySelector(".stepper-plus").addEventListener("click", () => {
    ...
    let stock = parseInt(ctrl.dataset.stock);   // FIXED
    ...
    // Require login
    if (!window.BLINKHUB_IS_LOGGED_IN) {
...
if (!isWishlistPage) {
    document.querySelectorAll('.wishlist-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            fetch('/BlinkHub-main/public/api/add-to-wishlist.php', {
                method: 'POST',
...
if (isWishlistPage) {
    ...
    btn.addEventListener('click', () => {
        ...
        let cart = loadCart(); // same cart as main page
...
// ====== The banner picture should only show in the All category

document.addEventListener('DOMContentLoaded', () => {
    const banner = document.getElementById('banner');
    const categoryButtons = document.querySelectorAll('.category-pill');

    if (!banner || !categoryButtons.length) return;
    ...
    products.forEach(product => {
        ...
        if (!category || productCategory === category) {
            product.style.display = '';     // 👈 restore original CSS display
        } else {
            product.style.display = 'none'; // 👈 hide only
        }
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const searchInput = document.querySelector(
        '#search, #searchInput, #search-box, input[name="search"], input[type="search"], input[type="text"]'
    );

    const searchForm = searchInput ? searchInput.closest('form') : null;

    // 1️⃣ Hide banner if URL has search
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has('search')) {
        banner.style.display = 'none';
    }

    // 2️⃣ Hide banner when typing
    if (searchInput) {
    ...
    // 3️⃣ Hide banner on Enter / submit
    if (searchForm) {
