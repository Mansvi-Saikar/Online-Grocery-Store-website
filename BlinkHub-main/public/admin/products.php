...
if (isset($_POST['action'])) {
    ...
    if (!$hasError) {
        if ($_POST['action'] === 'create') {
            $stmt = $mysqli->prepare("
                INSERT INTO products (name, mrp, price, image_url, category_id, stock, is_active)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ");
            if ($stmt) {
                $stock = (int)($_POST['stock'] ?? 0);
                $stmt->bind_param('sddsiii', $name, $mrp, $price, $img, $catId, $stock, $isActive);
                $ok = $stmt->execute();
                $stmt->close();
            ...
            if ($id > 0) {
                $stmt = $mysqli->prepare("
                    UPDATE products 
                    SET name = ?, mrp = ?, price = ?, image_url = ?, category_id = ?, stock = ?, is_active = ?
                    WHERE id = ?
                ");
                if ($stmt) {
                    $stock = (int)($_POST['stock'] ?? 0);
                    $stmt->bind_param('sddsiiii', $name, $mrp, $price, $img, $catId, $stock, $isActive, $id);
                    $ok = $stmt->execute();
                    $stmt->close();
            ...
            <div class="page-header">
                <div>
                    ...
                    <p class="page-subtitle">
                        Add, Delete, edit, enable or disable products in the store.
                    </p>
                </div>
